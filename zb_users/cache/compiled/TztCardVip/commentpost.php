...
<div class="mt15" id="divCommentPost">
	<div class="tzt-panel_hd">
		...
		<h3><i class="icon icon-feedback"></i><?php if ($user->ID>0) { ?> <?php  echo $user->StaticName;  ?> <?php }else{  ?> <?php  echo $user->StaticName;  ?><?php } ?> 我要留言</h3>
	</div>
		...
		<div class="post">
			<form id="frmSumbit" target="_self" method="post" action="<?php  echo $article->CommentPostUrl;  ?>" >
				...
				<div class="tzt-comment_input">
					<?php if ($zbp->Config('TztCardVip')->ArtComment2) { ?><p><input type="text" name="inpName" id="inpName" class="form-control text" value="<?php  echo $user->StaticName;  ?>" size="28" tabindex="1" placeholder="<?php  echo $lang['msg']['name'];  ?>" /></p><?php } ?>
					<?php if ($zbp->Config('TztCardVip')->ArtComment3) { ?><p class="center"><input type="text" name="inpEmail" id="inpEmail" class="form-control text" value="<?php  echo $user->Email;  ?>" size="28" tabindex="2" placeholder="<?php  echo $lang['msg']['email'];  ?>" /></p><?php } ?>
					<?php if ($zbp->Config('TztCardVip')->ArtComment4) { ?><p><input type="text" name="inpHomePage" id="inpHomePage" class="form-control text" value="<?php  echo $user->HomePage;  ?>" size="28" tabindex="3" placeholder="<?php  echo $lang['msg']['homepage'];  ?>" /></p><?php } ?>
				</div>
				<?php } ?>
