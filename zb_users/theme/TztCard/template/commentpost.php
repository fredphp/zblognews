...
<div class="mt15" id="divCommentPost">
	<div class="tzt-panel_hd">
		...
		<h3>我要留言{if $user.ID>0}({$user.StaticName}){/if}</h3>
	</div>
		...
		<div class="post">
			<form id="frmSumbit" target="_self" method="post" action="{$article.CommentPostUrl}" >
				...
				<div class="tzt-comment_input">
					<p><input type="text" name="inpName" id="inpName" class="form-control text" value="{$user.StaticName}" size="28" tabindex="1" placeholder="{$lang['msg']['name']}" /></p>
					<p class="center"><input type="text" name="inpEmail" id="inpEmail" class="form-control text" value="{$user.Email}" size="28" tabindex="2" placeholder="{$lang['msg']['email']}" /></p>
					<p><input type="text" name="inpHomePage" id="inpHomePage" class="form-control text" value="{$user.HomePage}" size="28" tabindex="3" placeholder="{$lang['msg']['homepage']}" /></p>
				</div>
				{/if}
